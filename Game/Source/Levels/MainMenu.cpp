...
void MainMenu::OnEnter()
{
    ...
    title->createComponent<TextComponent>(fontFilePath, "2D Engine - Horror Engine", sf::Color::Red, sf::Color::Black, 2.f);

    subTitle->createComponent<Transform>(static_cast<float>(windowWidth) / 2, 130.f);
    subTitle->createComponent<TextComponent>(fontFilePath, "Made by istinox", sf::Color::Red, sf::Color::Black, 2.f, 15);

    button->createComponent<Transform>(static_cast<float>(windowWidth) / 2, 300.f);
    button->createComponent<SpriteRenderer>(buttonFilePath);
